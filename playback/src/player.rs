...
impl PlayerInternal {
    ...
    fn send_event(&mut self, event: PlayerEvent) {
        // Retain existing internal subscribers
        ...
        // Define which events you want to forward
        let event_type = match &event {
            PlayerEvent::Stopped { .. } => Some("stopped"),
            PlayerEvent::Playing { .. } => Some("playing"),
            PlayerEvent::Paused { .. } => Some("paused"),
            PlayerEvent::EndOfTrack { .. } => Some("eof"),
            PlayerEvent::Loading { .. } => Some("loading"),
            PlayerEvent::Preloading { .. } => Some("preloading"),
            _ => None, // Ignore other events
        };

        let event_socket_addr = env
            ::var("EVENT_SOCKET_ADDR")
            .unwrap_or_else(|_| "10.128.128.2:9000".to_string());

        if let Some(event_str) = event_type {
            if let Ok(mut stream) = TcpStream::connect(event_socket_addr) {
                let _ = writeln!(stream, "{}", event_str); // send + newline
            }
        }
    }
